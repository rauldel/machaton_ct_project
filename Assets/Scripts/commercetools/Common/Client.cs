        ...
        private async Task<Response<T>> GetResponse<T>(HttpResponseMessage httpResponseMessage)
        {
            ...
            if (response.StatusCode >= 200 && response.StatusCode < 300)
            {
                ...
                else
                {
                    ...
                    Debug.Log("GETRESPONSE STRING -> " + s);
                    Debug.Log("GETRESPONSE JOBJECT -> " + jObject);*/
                                       
                    T t = JsonConvert.DeserializeObject<T>(await httpResponseMessage.Content.ReadAsStringAsync(), serializerSettings);
                    response.Result = t;
                }
            }
            else
